...
impl From<Graphic> for GraphicDTO {
    fn from(g:Graphic) -> Self {
        Self{
            ...
            start:g.start.split(",").map(|num| num.parse::<f64>().unwrap()).collect::<Vec<_>>(),
            end:g.end.split(",").map(|num| num.parse::<f64>().unwrap()).collect::<Vec<_>>(),
            content:g.content,
            style:if g.style.is_some() { Some(serde_json::from_str(&g.style.unwrap()).unwrap()) } else { None },
