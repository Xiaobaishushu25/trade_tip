...
use crate::utils::stock_util::{
    calculate_ago_days_with_num, calculate_ago_days_with_str, calculate_ago_minutes,
    compute_mul_ma, compute_single_ma, get_market_by_code,
};
use anyhow::anyhow;
...
pub async fn handle_stock_livedata(
    ...
    for code in codes {
        let op_live_data = now_data
            .get_mut(code);
        if op_live_data.is_none() {
            continue;
        }
        let live_data = op_live_data.unwrap();
...
pub async fn handle_can_t(
    ...
    for code in codes {
        ...
        } else if (res_count as f64) / (num as f64) < 0.3
            && percentage_change < data_config.down_t_trigger
        {
...
async fn test_handle_new_stock() {
    init_http().await;
    init_db_coon().await;
    handle_new_stock("512690", "白酒ETF",false).await.unwrap();
}
#[tokio::test]
